...
fun main() {
    val fontsDir = File(".\\src\\fonts")
    if (!fontsDir.exists() || !fontsDir.isDirectory) {
        println("Error: Directory does not exist or is not a directory.")
        return
    }
    fontsDir.listFiles { _, name -> name.endsWith(".ttf") && !name.startsWith("MenloNL") }
            ?.forEach {
            ...
            } ?: println("NO TTF FILE FOUND IN SPECIFIED DIRECTORY!!!!!")
}
...
fun generateNoLigaturesFont(file: File, doc: Document) {
    val nlName = file.nameWithoutExtension.replace("DejaVuSansMono", "DejaVuSansMonoNL")
    val ttx = "$nlName.ttx"
    val ttf = "$nlName.ttf"
